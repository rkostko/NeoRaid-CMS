...
class FileTreeManager implements ITreeManager 
{
	...
	public function __construct($fileRoot, $editable) {
		if (is_dir($fileRoot)) 
		{
	...
	public function getElementList($ownerEl, $pageName) 
	{
		...
		if (is_dir($fullPath) && $handle = opendir($fullPath))
		{
		   	...
		   	while (false !== ($file = readdir($handle))) 
			{
				...
				$supp = NULL;
				if($this->editable && !in_array($file, $this->editable) && empty($realOwnerEl))	continue;				 
				...
				//echo "realOwnerEl: ".$realOwnerEl;
				//if($this->editable && empty($realOwnerEl) && !in_array($file, $this->editable)) continue;
