...
switch($action)
{
	...
	{	
		...
		{				
            ...
            $type = (isset($_POST['type']) && !empty($_POST['type']))? checkVariable($_POST['type']) : false;
			$galleryId = null;			
		    if($slave == 1)
		    {
                if($type == 'galleries') $itemId = $galleryManager->createGallery($name);
                elseif($type == 'articles') $itemId = $articleManager->createArticle($name);
		    }
			
			
			$out = $treeManager->insertElement($name, $ownerEl, $slave, $itemId, $type);						
		}
		else {
			$out = FAILED; 
		}
    ...
    case "hideElement":
    {
    	 ...
    	 */
		if (isset($_POST['isVisible']) && !empty($_POST['isVisible']) &&
		    isset($_POST['elementId']) && !empty($_POST['elementId']) &&
		    isset($_POST['ownerEl']) && $_POST['ownerEl'] != NULL)
		{			
			$isVisible = checkVariable($_POST['isVisible']);
			$isVisible = ($isVisible == 'hidden')? 1 : 0;
			$elementId = checkVariable($_POST['elementId']); 			
			$ownerEl = checkVariable($_POST['ownerEl']);
			$out = $treeManager->hideElement($isVisible, $elementId, $ownerEl);
		}                         
		else 
		{
			$out = FAILED;	
