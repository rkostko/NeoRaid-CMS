...
function TreeOperations()
{
	...
	this.inputText = "<input type='text' id='inputText' maxlength='256'>";
...
//**********************************************************
	this.trAddElement = function(result)
	{
        ...
        $('#inputText').parent().attr('rel', treeOps.itemType);
	...
	/////////////////////////////////////////////////////////////
	this.addElementReq = function(folder, type)
		...
		$('#inputText').bind("keypress", 
			 ...
			 {												
				...
				{	
					...
					var params = encodeURI("action=insertElement"+"&name="+name+"&ownerEl="+ownerEl+"&slave="+slave+"&type="+type);
//alert(params);                   
...
/******************************************************	
	UKRYWANIE ELEMENTOW
*******************************************************/	
	this.trHideElement = function(result)
	{
		if (result != operationFailed)	
		{		
			var selElement = treeOps.trGetSelected();
			var selSpan = $(selElement).find('span');
			if($(':first', selSpan).get(0) != undefined && $(':first', selSpan).get(0).tagName.toLowerCase() == 'img')
			{
				$(':first', selSpan).remove();
			}
			else
			{
				$(selSpan).prepend('<img style="vertical-align:bottom; margin-right:2px" src="js/jquery/plugins/simpleTree/images/stop.png" border="0" />');
			}
		}
		else
		{
	...
	/////////////////////////////////////////////////////
	this.hideElementReq = function()
	{	
		if ( treeOps.isTreeBusy() == true || treeOps.trGetSelectedWithAlert() == null) 
		{
			return;
		}	
		treeOps.treeBusy = true;
		var ownerEl = treeOps.trGetSelected().parent().parent().attr('id');
		var visible = $('span', treeOps.trGetSelected()).attr("rel");
	 	var params = "action=hideElement&isVisible="+visible+"&elementId="+treeOps.trGetSelected().attr('id')
	 				 + "&ownerEl=" + ownerEl;
		treeOps.ajaxReq(params, structureManagerURL, treeOps.trHideElement);
	}
