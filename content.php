	...
	private function _getMovies($buffer)
	{
		if(preg_match_all("#{movies}(.*?){/movies}#s", $buffer, $matches, PREG_PATTERN_ORDER) > 0)
		{
			...
			foreach ($matches[0] as $match)
			{
				...
				FROM `movies`
				WHERE ".$where;
				if(!is_numeric($_movie_))
				{
					$where = "path = '".$_movie_."'";
				}
				else
				{
					$where = "id = ".(int) $_movie_;
				}
				$query .= $where;
				$result = mysql_query($query, $this->link);
				...
				if(mysql_num_rows($result) > 0)
				{
					...
					foreach($movies as $movie)
					{
						...
						if(preg_match('/http:\/\/www\.youtube\.com\/v\[a-z0-9_]+?\?version=/', $movie['url']))
						{
						...
						elseif(preg_match('/http:\/\/www\.youtube\.com\/watch\?v=/is', $movie['url']))
						{
							$movie['url'] = preg_replace('/(http:\/\/www\.youtube\.com\/)(watch\?v=)(.+?)/is', '$1'."embed/".'$3', $movie['url']);
							$movie['url'] .= "?wmode=transparent";
							$html .= '<iframe width="'.$movie['width'].'" height="'.$movie['height'].'" src="'.$movie['url'].'" frameborder="0" allowfullscreen></iframe>';
						}
						$html .= '</div>';
					}
				}
	...
	private function _getNews($buffer)
	{
		if(preg_match_all("#{news}(.*?){/news}#s", $buffer, $matches, PREG_PATTERN_ORDER) > 0)
		{
			...
			if(mysql_num_rows($result) > 0)
			{
				...
				{
					$html .= '<div class="newsitem" style="position:relative">';
					
					$html .= '<div class="newsdate" style="position: absolute; top:0px; left:0px; padding:3px;">';
					$html .= $item['date'];
					$html .= '</div>';
					/*$html .= '<table class="newstable" cellpadding="5" border="1">';
					$html .= '<tr>';
					...
					if($item['object'])
					{
						$html .= '<td class="newsobject" style="padding-top:30px">';
						$html .= '{objects}'.$item['object'].'{/objects}';
						$html .= '</td>';
					}
					$html .= '<td class="newsheadline">';
					
					$html .= '<div class="newstitle" style="margin-bottom:10px;">'.$item['title'].'</div>';
					...
					if(file_exists($path.$slash.$item['headline'].".html"))
					{
						...
						elseif(preg_match('/{movies}.+?{\/movies}/sm', $headline))
						{
							$html .= $this->_getMovies($headline);
						}
						else
						{
							$html .= $headline;
